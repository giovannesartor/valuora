...
class TestWACC:
    def test_basic_wacc(self):
        wacc = calculate_wacc(beta=1.0)
        assert 0.10 < wacc < 0.30
        assert isinstance(wacc, float)

    def test_higher_beta_higher_wacc(self):
        wacc_low = calculate_wacc(beta=0.8)
        wacc_high = calculate_wacc(beta=1.5)
        assert wacc_high > wacc_low

    def test_debt_reduces_wacc(self):
        wacc_no_debt = calculate_wacc(beta=1.0, debt_ratio=0.0)
        wacc_with_debt = calculate_wacc(beta=1.0, debt_ratio=0.3)
...
class TestFCFProjection:
    def test_projects_5_years(self):
        fcf = project_fcf(revenue=1_000_000, net_margin=0.15, growth_rate=0.10)
        assert len(fcf) == 5
    ...
    def test_positive_fcf_with_good_margins(self):
        fcf = project_fcf(revenue=1_000_000, net_margin=0.20, growth_rate=0.10)
        for year in fcf:
            assert year["fcf"] > 0

    def test_growth_decelerates(self):
        fcf = project_fcf(revenue=1_000_000, net_margin=0.15, growth_rate=0.20)
...
class TestEnterpriseValue:
    def test_positive_ev(self):
        fcf = project_fcf(revenue=1_000_000, net_margin=0.15, growth_rate=0.10)
        tv = calculate_terminal_value(last_fcf=fcf[-1]["fcf"], wacc=0.15)
        ev = calculate_enterprise_value(fcf, wacc=0.15, terminal_value=tv)
...
class TestSectorBeta:
    def test_known_sector(self):
        assert get_sector_beta("tecnologia") == 1.3

    def test_unknown_sector(self):
        assert get_sector_beta("xyz_inventado") == 1.0

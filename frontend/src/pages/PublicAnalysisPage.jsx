...
export default function PublicAnalysisPage() {
  ...
  useEffect(() => {
    ...
    const equityFmt = data.equity_value != null
      ? Number(data.equity_value).toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 })
      : null;
    const title   = `${data.company_name} — Business Valuation | Valuora`;
    const description = equityFmt
      ? `Estimated value of ${data.company_name}: ${equityFmt}. Analysis generated by Valuora using DCF methodology and market multiples.`
      : `View the valuation analysis of ${data.company_name} generated by Valuora.`;
    ...
    setMeta('og:image', 'https://valuora.online/og-cover.png');
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center px-4 ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
        <div className={`w-full max-w-sm rounded-2xl border p-8 shadow-xl ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
          ...
          <h2 className={`text-lg font-bold text-center mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>Protected analysis</h2>
          <p className={`text-sm text-center mb-6 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
            The owner protected this analysis with a password.
          </p>
          <form onSubmit={handlePasswordSubmit} className="space-y-3">
            ...
            >
              {passwordLoading ? 'Verifying...' : 'Access analysis'}
            </button>
          </form>
        </div>
      </div>
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center px-4 ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
        <div className="text-center max-w-sm">
          ...
          <h2 className={`text-xl font-bold mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>Invalid link</h2>
          <p className={`text-sm ${muted}`}>{error}</p>
          <a href="https://valuora.online" className="inline-block mt-6 text-sm text-emerald-500 hover:text-emerald-400 font-medium">
            Go to Valuora →
          </a>
        </div>
      </div>
    ...
    <div className={`min-h-screen transition-colors ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className={`${card} mb-6`}>
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div className="text-right">
              <p className={`text-xs font-medium uppercase tracking-wide ${muted}`}>Estimated Value</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
          {[
            { label: 'Risk Score', value: `${Number(data.risk_score || 0).toFixed(1)}/100`, icon: Shield, color: 'text-red-400', bg: isDark ? 'bg-red-500/10' : 'bg-red-50' },
            { label: 'Maturity Index', value: `${Number(data.maturity_index || 0).toFixed(1)}/100`, icon: Gauge, color: 'text-blue-400', bg: isDark ? 'bg-blue-500/10' : 'bg-blue-50' },
        ...
        {data.ai_analysis && (
          <div className={`${card} mb-6`}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className={`font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>Valuora Intelligence Analysis</h2>
            </div>
        ...
        <div className={`${card} text-center`}>
          <p className={`text-sm ${muted} mb-3`}>Want to know the real value of your company?</p>
          <a
            href="https://valuora.online"
